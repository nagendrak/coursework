...
PROGRAM multiRegionShield
  ...
  READ*, nParticles
  PRINT*, 'Type of free flight: (1) Path length (2) MFP'
  READ*, freeFlightFlag
...
!   PRINT*, 'particle number:', iParticle
    ...
    DO WHILE (parStatus .EQ. 0)
      ...
      IF (freeFlightFlag .EQ. 1) THEN
        CALL freeFlightPL
      ELSE IF (freeFlightFlag .EQ. 2) THEN
        CALL freeFlightMFP
      END IF
...
SUBROUTINE freeFlightMFP
  ...
  INTEGER :: iRegion
  REAL :: randNum, nMFPs, nMaxMFPs

! PRINT*, 'in freeFlightMFP'
...
! PRINT*, 'b =', nMFPs
  ...
  IF (scatterAngle .GT. 0.0)  THEN
    DO iRegion = currentRegion, nShields
      nMaxMFPs = (shld(iRegion)%xMax-xLocation)                     &
          *shld(iRegion)%sigmaTotal/scatterAngle
!     PRINT*, 'bmax =', nMaxMFPs
      IF (nMFPs .LT. nMaxMFPs) THEN
        xLocation = xLocation                                       &
            + nMFPs/shld(iRegion)%sigmaTotal*scatterAngle
        EXIT
      ELSE
        xLocation = shld(iRegion)%xMax
        nMFPs = nMFPs - nMaxMFPs
      END IF
    END DO
  ELSE
    DO iRegion = currentRegion, 1, -1
      nMaxMFPs = (shld(iRegion)%xMin-xLocation)                     &
          *shld(iRegion)%sigmaTotal/scatterAngle
!     PRINT*, 'bmax =', nMaxMFPs
      IF (nMFPs .LT. nMaxMFPs) THEN
        xLocation = xLocation                                       &
  ...
  IF (currentRegion .GT. nShields) THEN
    ...
    nParLeak = nParLeak + 1
!   PRINT*, 'Particle leaked'
  ELSE IF (currentRegion .LT. 1) THEN
    parStatus = 2
    nParRef = nParRef + 1
!   PRINT*, 'Particle reflected'
  ELSE
